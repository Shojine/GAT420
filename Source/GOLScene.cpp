...
void GOLScene::Update()
{
	...
	//clears the next cells for the next iteration

	if (m_input.GetKeyDown(SDL_SCANCODE_SPACE))
	{
		for (int i = 0; i < nextCells->m_data.size(); i++)
		{
			nextCells->m_data[i] = (random(80) == 0) ? 1 : 0;
		}
	}
	...
	for (int y = 1; y < nextCells->m_height - 1; y++)
	{
		for (int x = 1; x < nextCells->m_width - 1; x++)
		{
			...
			// do the game of life rule 
			
			uint8_t currentState = currentCells->Read(x, y);
			if (currentState)
			{
				//we are alive, stay alive if we have 2 or 3 neighbors, if not then dead
				//uint8_t nextState = (count != 1 && count <= 3) ? 1 : 0;
	...
	for (int i = 0; i < m_cellsB->m_data.size(); i++)
	{
		m_framebuffer->m_buffer[i] = (m_cellsA->m_data[i]) ? white : black;
	}for (int i = 0; i < m_cellsB->m_data.size(); i++)
	{
		m_framebuffer->m_buffer[i] = (m_cellsB->m_data[i]) ? white : black;
	}
